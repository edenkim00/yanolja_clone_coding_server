const nunjucks = require('nunjucks');
module.exports = function(app){
    const user = require('./userController');
    const jwtMiddleware = require('../../../config/jwtMiddleware');
    const passport = require('passport');
    app.set('view engine','html');
    nunjucks.configure('views',{
        express:app,
    })
    // 0. 테스트 API
    // app.get('/app/test', user.getTest)

    // 1. 유저 생성 (회원가입) API
    app.post('/app/user', user.postUsers);

    // 2. 유저 조회 API (+ 검색)
    app.get('/app/user',user.getUsers);

    // 3. 특정 유저 조회 API
    app.get('/app/user/:userId', jwtMiddleware,user.getUserById);


    // TODO: After 로그인 인증 방법 (JWT)
    // 로그인 하기 API (JWT 생성)
    app.post('/app/login', user.login);
    app.post('/app/email-validation',user.emailvalidation);
    app.post('/app/phone-validation',user.phonevalidation);

    // 회원 정보 수정 API (JWT 검증 및 Validation - 메소드 체이닝 방식으로 jwtMiddleware 사용)
    app.patch('/app/user/:userId', jwtMiddleware, user.patchUsers)

    app.patch('/app/user/:userId', jwtMiddleware, user.deleteUsers);
    app.get('/app/kakaologin', user.kakaologin);
    // app.get('/app/kakaologin/callback', passport.authenticate('kakao'), user.kakaologincallback);
    app.get('/app/kakaologin/callback', passport.authenticate('kakao', {
        successRedirect : '/',failureRedirect: '/',
    }), (req, res) => {
        console.log(req);
        res.redirect('/');
    });
    app.post('/app/kakaologin',user.kakaologinAccess);


};


// TODO: 자동로그인 API (JWT 검증 및 Payload 내뱉기)
// JWT 검증 API
// app.get('/app/auto-login', jwtMiddleware, user.check);

// TODO: 탈퇴하기 API